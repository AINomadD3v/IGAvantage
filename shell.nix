...
let
  ...
  pythonDepsList = [
    "openai" "pytesseract" "opencv-python" "dnspython" "gdown" "pyaml"
    "pyyaml" "requests" "lxml" "pillow" "retry" "progress" "xmltodict"
    "six" "logzero" "packaging" "whichcraft" "pbr" "pyelftools"
    "pycryptodome" "pytest" "isort" "pytest-cov" "ipython" "coverage"
    "inflection" "pydantic" "poetry-core" "poetry-dynamic-versioning"
    "python-dotenv" "opencv4" "scikitimage" "pandas" "google-auth"
    "google-auth-oauthlib" "google-auth-httplib2" "google-api-python-client"
    "scikit-learn" "deprecated" "filelock" "deprecation"
  ];

  # Deduplicated pythonDeps
  pythonDeps = builtins.map addPythonPackage (builtins.attrNames (builtins.listToAttrs (map (n: { name = n; value = null; }) pythonDepsList)));
  ...
  nextcaptcha-python = pythonPackages.buildPythonPackage rec {
    ...
    src = pkgs.fetchFromGitHub {
      ...
      rev = "v${version}";
      sha256 = "sha256-GlBGfXgIpJYcg/zRN5YRXTFDKL+q9xV7XSeHXcCzRjY=";
    };
