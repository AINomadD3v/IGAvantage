...
# In Shared/VPN/nord.py


def main_flow(d):
    """
    Handles the entire NordVPN IP rotation process with robust waiting logic.
    """
    ...
    while time.time() < deadline:
        ...
        if d.xpath(xpath_connected).exists:
            # Verify the IP has actually changed to avoid false positives
        ...
        # The background watcher will handle the "taking longer" message automatically.
        # This main loop just needs to keep polling patiently.
    ...
    # Re-query the final status to log it
